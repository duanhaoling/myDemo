...
public class ImageHelper {
    ...
    public Bitmap loadImage(String filePathOrUrl) {
        ...
        if (filePathOrUrl.startsWith("http")) {
            ...
            DevUtil.v(TAG, String.format("DiskCache hit(Nomal) path:%s", filePathOrUrl));
     ...
     * 判断图片是否已经磁盘缓存
     *
     * @param filePathOrUrl 图片的url或者加载的最原始的本地路径(http://a.com/a.jpg或者/data/data/xxx
     *                      .com/cache/a.jpg)
     * @return
     */
    ...
    public File getReSizeFile(File imageFile, int width, int height) {
        try {
            File cacheFile = new File(AppContext.getInstance().getCacheDir(),
