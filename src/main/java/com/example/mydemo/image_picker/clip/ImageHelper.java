...
import com.ldh.androidlib.utils.MD5Util;
...
public class ImageHelper {
    ...
    private Bitmap getInSampleAndCutBitmap(String filePathOrUrl, int cutWidth, int cutHeight, String cachePath) {
        ...
        if (isDiskCached(filePathOrUrl, cutWidth, cutHeight, cachePath, true)) {
            ...
            DevUtil.v("imageHelper", String.format("DiskCache hit(InSampleAndCut) path:%s", filePath));
    ...
    public Bitmap getInSampleBitmap(String filePath, int maxWidth, int maxHeight, String cachePath) {
        ...
        // 会产生问题，后续拍照生成的略缩图因为filePath全是返回的缓存的第一张图
        if (isFileNameInCache(bitmapFileNameMd5, cachePath)) {// 已经有缓存
            DevUtil.v("imageHelper", String.format("DiskCache hit(InSample) path:%s/%s", cachePath, bitmapFileNameMd5));
    ...
    public Bitmap getRemotePicInSampleBitmap(String url, int maxWidth, int maxHeight, String cachePath) {
        ...
        if (isDiskCached(url, maxWidth, maxHeight, cachePath)) {
            ...
            DevUtil.v("imageHelper", String.format("DiskCache hit(InSample) path:%s", filePath));
            return BitmapFactory.decodeFile(filePath);
        }
    ...
    public Bitmap loadImage(String filePathOrUrl) {
        ...
        if (filePathOrUrl.startsWith("http")) {
            ...
            DevUtil.v("Imagehelper", String.format("DiskCache hit(Nomal) path:%s", filePathOrUrl));
...
//            }
//        } else {
//            DevUtil.v("imageHelper", String.format("DiskCache hit(Nomal) path:%s", imagePath));
//        }
...
//     * @throws IOException
//     */
//    public byte[] loadImageFromNetwork(String url) throws IOException {
...
//    }
    ...
    public Bitmap loadImage(String filePathOrUrl, int width, int height, String cachePath, boolean isNeedCut) {
        ...
        switch (imageLoadType) {
            case Nomal:
                DevUtil.v("imageHelper", String.format("loadImage:Nomal task:%s", this));
                ret = loadImage(filePathOrUrl);
                break;

            case InSample:
                DevUtil.v("imageHelper", String.format("loadImage:InSample task:%s", this));
                ret = getInSampleBitmapUseFilePathOrUrl(filePathOrUrl, width, height, cachePath);

                break;

            case InSampleAndCut:
                DevUtil.v("imageHelper", String.format("loadImage:InSampleAndCut task:%s", this));
